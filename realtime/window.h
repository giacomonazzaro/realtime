...
struct Window {
    ...
    void click(bool left, bool pressed) {
        callbacks.click(*this, left, pressed);
    }
    void scroll(float amount) { callbacks.scroll(*this, amount); }
    void focus(int focused) { callbacks.focus(*this, focused); }
};

void init_window(Window& win, const vec2i& size, const string& title, bool visible = true);
...
void run_draw_loop(Window& win, function<void(Window&)> draw,
                   bool wait = false);
...
bool gui_slider(Window& win, const char* lbl, float& value, float min,
                float max);
bool gui_slider(Window& win, const char* lbl, vec2f& value, float min,
                float max);
bool gui_slider(Window& win, const char* lbl, vec3f& value, float min,
                float max);
bool gui_slider(Window& win, const char* lbl, vec4f& value, float min,
                float max);
...
bool gui_combobox(Window& win, const char* lbl, int& idx, int num,
                  const std::function<std::string(int)>& labels,
...
void gui_histogram(Window& win, const char* lbl,
